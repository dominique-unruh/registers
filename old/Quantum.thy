theory Quantum
  imports CatLValue_Syntax 
    QHLProver.Gates QHLProver.Quantum_Hoare
begin


typedef ('a::enum) vector = "carrier_vec CARD('a) :: complex vec set"
  apply (rule exI[of _ \<open>zero_vec CARD('a)\<close>])
  by simp
setup_lifting type_definition_vector

lift_definition vector_idx :: "'a::enum vector \<Rightarrow> 'a \<Rightarrow> complex" is
  undefined
  sorry
...
locale grover_state =
  fixes f :: "'a::enum \<Rightarrow> bool"  (* characteristic function, only need values in [0,N). *)
  assumes dimM: "{i. f i} \<noteq> {}"
              "{i. f i} \<noteq> UNIV"
begin
...
lemma M_ge_0 [simp]: "0 < M" sorry

lemma M_neq_0 [simp]: "M \<noteq> 0" by simp

lemma M_le_N [simp]: "M < N" sorry

lemma M_not_ge_N [simp]: "\<not> M \<ge> N" using M_le_N by arith

lift_definition \<psi> :: "'a vector" is
  "Matrix.vec N (\<lambda>i. 1 / sqrt N) :: complex vec"
  unfolding N_def by simp

lemma \<psi>_eval:
  "vector_idx \<psi> a = 1 / sqrt N"
  sorry

lemma \<psi>_inner:
  "inner_prod (Rep_vector \<psi>) (Rep_vector \<psi>) = 1"
  apply transfer
  apply (simp add: \<psi>_eval scalar_prod_def)
  by (smt of_nat_less_0_iff of_real_mult of_real_of_nat_eq real_sqrt_mult_self)
 
lemma \<psi>_norm:
  "vec_norm (Rep_vector \<psi>) = 1"
  apply transfer
  by (simp add: \<psi>_eval vec_norm_def scalar_prod_def)

lift_definition \<alpha> :: "'a vector" is
  "Matrix.vec N (\<lambda>i. if f (Enum.enum!i) then 0 else 1 / sqrt (N - M)) :: complex vec"
  unfolding N_def by simp

lemma \<alpha>_eval:
  "vector_idx \<alpha> i = (if f i then 0 else 1 / sqrt (N - M))"
  sorry

lemma \<alpha>_inner:
  "inner_prod (Rep_vector \<alpha>) (Rep_vector \<alpha>) = 1"
  apply transfer
(*   apply (simp add: scalar_prod_def \<alpha>_eval)
  ...
  subgoal by (metis of_nat_0_le_iff of_real_of_nxat_eq of_real_power power2_eq_square real_sqrt_pow2)
...
lift_definition \<beta> :: "'a vector" is
  "Matrix.vec N (\<lambda>i. if f (Enum.enum!i) then 1 / sqrt M else 0) :: complex vec"
  unfolding N_def by simp

lemma \<beta>_eval:
  "vector_idx \<beta> i = (if f i then 1 / sqrt M else 0)"
  sorry

lemma \<beta>_inner:
  "inner_prod (Rep_vector \<beta>) (Rep_vector \<beta>) = 1"  
(* 
...
lemma alpha_beta_orth:
  "inner_prod (Rep_vector \<alpha>) (Rep_vector \<beta>) = 0"
  apply transfer
  unfolding \<alpha>_def \<beta>_def 
  by (simp add: scalar_prod_def)

lemma beta_alpha_orth:
  "inner_prod (Rep_vector \<beta>) (Rep_vector \<alpha>) = 0"
  apply transfer
