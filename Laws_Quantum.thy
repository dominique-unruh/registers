...
theory Laws_Quantum
  imports Quantum "HOL-Library.Rewrite" Misc
begin
...
lemma left_tensor_hom[simp]: "maps_hom ((\<otimes>) a :: 'b::finite domain_end \<Rightarrow> _)" 
  for a :: "'a::finite domain_end"
  apply (rule maps_2hom_right)
  by (simp add: maps_2hom_right)

lemma right_tensor_hom[simp]: "maps_hom (\<lambda>a::'a::finite domain_end. (\<otimes>) a b)"
  for b :: "'b::finite domain_end"
  by (simp add: maps_2hom_left)

lemma tensor_extensionality3: 
  fixes F G :: \<open>('a::finite\<times>'b::finite\<times>'c::finite, 'd::finite) maps_hom\<close>
...
term \<open>(tensor_maps_hom swap id) \<circ> swap \<circ> assoc \<circ> (tensor_maps_hom swap id) \<circ> (swap :: ('a::finite\<times>('b::finite\<times>'c::finite), _) maps_hom)\<close>
...
(* TODO: needed? *)
definition compatible0 :: \<open>('a::finite,'c::finite) maps_hom \<Rightarrow> ('b::finite,'c) maps_hom \<Rightarrow> bool\<close> where
  \<open>compatible0 F G \<longleftrightarrow> (\<forall>a b. F a \<circ>\<^sub>d G b = G b \<circ>\<^sub>d F a)\<close>

definition compatible :: \<open>('a::finite,'c::finite) maps_hom \<Rightarrow> ('b::finite,'c) maps_hom \<Rightarrow> bool\<close> where
...
definition pair :: \<open>('a::finite,'c::finite) maps_hom \<Rightarrow> ('b::finite,'c) maps_hom \<Rightarrow> ('a\<times>'b, 'c) maps_hom\<close> where
